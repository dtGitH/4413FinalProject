	...
	public UserProfile getProfile(String userUuid) {
		...
		try (Connection connection = getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					return new UserProfile(userUuid, rs.getString("first_name"), rs.getString("last_name"),
							rs.getString("phone"), rs.getString("dob"), rs.getString("gender"), rs.getString("address"),
	...
	public boolean createProfile(UserProfile profile) {
		...
		try (Connection connection = getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
			...
			if (profile.getDob() != null) {
				...
				ps.setNull(5, java.sql.Types.DATE); // This is the key change
			}
	...
	public boolean updateProfile(UserProfile profile) {
		...
		try (Connection connection = getConnection(); PreparedStatement ps = connection.prepareStatement(sql)) {
			...
			if (profile.getDob() != null) {
				...
				ps.setNull(4, java.sql.Types.DATE); // This is the key change
			}
